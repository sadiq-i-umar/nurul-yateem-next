...
import { Box, Button, Divider, Grid, LinearProgress, Paper, Typography, Dialog, DialogActions, DialogContent, DialogTitle, TextField, FormControl, InputLabel, Select, MenuItem, FormGroup, FormControlLabel, Checkbox, IconButton } from "@mui/material";
...
const OrphanSponsorshipCard: React.FC<OrphanSponsorshipCardProps> = ({ cardData }) => {
  ...
  return (
    <Box sx={{ paddingTop: "10px", paddingRight: "0px", paddingBottom: "10px", paddingLeft: "0px" }}>
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap" }}>
                ...
                <Box>
                  <Box sx={{ display: "flex", alignItems: "center", flexWrap: "wrap" }}>
                    <Box>
                      <Typography sx={{ fontSize: "16px", wordWrap: "break-word" }}>{card.name}</Typography>
                    </Box>
                    <Box>
                      <PillWithDot
                        text={card.date}
                  ...
                  <Box sx={{ display: "flex", mb: "20px" }}>
                    {card.needs.map((need) => (
                      <Box key={need}>
                        <TextOnlyPill text={need} bgColor="#EFF8FF" color="#175CD3" />
                      </Box>
                    ))}
                  </Box>
                  <Box sx={{ mb: "20px" }}>
        ...
        <DialogContent>
          <Box sx={{ p: 3 }}>
            ...
            <FormControl fullWidth margin="normal">
              ...
              <Typography variant="caption" sx={{ position: "absolute", right: "16px", top: "10px", color: "#3863FA" }}>NGN</Typography>
            </FormControl>
  ...
  <FormGroup>
    <FormControlLabel
      control={<Checkbox checked={donationType.clothing} onChange={handleChangeDonationType} name="clothing" />}
      label={<Typography sx={{ fontSize: "13px" }}>Clothing</Typography>}
    />
    <FormControlLabel
      control={<Checkbox checked={donationType.education} onChange={handleChangeDonationType} name="education" />}
      label={<Typography sx={{ fontSize: "13px" }}>Education</Typography>}
    />
    <FormControlLabel
      control={<Checkbox checked={donationType.health} onChange={handleChangeDonationType} name="health" />}
      label={<Typography sx={{ fontSize: "13px" }}>Health</Typography>}
    />
  </FormGroup>
</Box>
  ...
  >
    Send Donations
        ...
        <Box sx={{ p: 2, textAlign: "center", color: "#475367", fontSize: "14px" }}>
          You will be redirected to a Third-party app where you will be charged for your donations. Be rest assured that allyour details are safe.        </Box>
      </Dialog>
    </Box>
  );
};
